...
function review(over: Partial<YearReview> = {}): YearReview {
  return {
    ...
    issueCount: 0,
    counts: { needs_receipt: 0, missing_context: 0, gift_cap: 0, duplicate: 0, mixed_account: 0, vehicle_method: 0, vague_memo: 0 },
    ...over,
  };
}
...
test('formatYearReview: gaps list pluralized phrases + cleanup link', () => {
  const out = formatYearReview(
    review({
      issueCount: 3,
      counts: { needs_receipt: 2, missing_context: 1, gift_cap: 0, duplicate: 0, mixed_account: 0, vehicle_method: 0, vague_memo: 0 },
    }),
    URL,
  );
...
test('formatYearReview: singular gap noun', () => {
  const out = formatYearReview(
    review({
      issueCount: 1,
      counts: { needs_receipt: 1, missing_context: 0, gift_cap: 0, duplicate: 0, mixed_account: 0, vehicle_method: 0, vague_memo: 0 },
    }),
    URL,
  );
